...
func DivideIntHandler(logger *slog.Logger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Method != "POST" {
			respond.WriteError(logger, w, http.StatusMethodNotAllowed, "only POST allowed")
			return
		}
		var req models.DivideRequestInt
		...
		if *req.B == 0 {
			respond.WriteError(logger, w, http.StatusBadRequest, "division by zero is not allowed")
			return
		}
...
func DivideFloatHandler(logger *slog.Logger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Method != "POST" {
			respond.WriteError(logger, w, http.StatusMethodNotAllowed, "only POST allowed")
			return
